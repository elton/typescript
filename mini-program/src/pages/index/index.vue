<template>
  ...
  <view class="yuyue">
    <image src="https://diancan-1252107261.cos.accelerate.myqcloud.com/yiliao/nav-yiyu.png" mode="aspectFill"
      @click="jumpToSelfTesting('抑郁测评专业版', 0, '001')" />
  </view>
  <!-- 疫苗预约 -->
  <view class="gongge">
  ...
  <view class="self-test" v-if="selfTest.length > 0">
    <view class="sele-test-top sele-test-flex sele-test-one sele-test-back" v-for="(item, index) in [selfTest[0]]"
      :key="index" @click="jumpToSelfTesting(item.name, index, '001')">
    ...
    <view class="sele-test-top sele-test-flex sele-test-back" v-for="(item, index) in selfTest.slice(1)" :key="index"
      @click="jumpToSelfTesting(item.name, index, '002')">
      <view class="sele-test-view">
        <text class="top-title">{{ item.name }}</text>
...
// 健康自测跳转
const jumpToSelfTesting = (name: string, index: number, type: string) => {
  if (type === '001') {
    uni.navigateTo({
